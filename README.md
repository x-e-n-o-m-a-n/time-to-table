...
Программа - калькулятор для расчёта графиков работ. Сделан на Tauri чтоб работал как нормальная программа, а не в браузере.

Цель создания - ускорение ввода данных, после выгрузки .xlsx данные из него копируются -> ручками через ctrl-c/v.
...
- Экспорт в EXCEL (.xlsx) - выгружает всю историю расчётов в файл, формулы работают (можно менять время и пересчитается)
...
| F5  | Очистить - сбрасывает некоторые поля... |
| ctrl z/y | undo/redo |
...
- **Защита от Excel-формульной инъекции** - `excelSanitizeCell` для текстовых пользовательских данных при экспорте в Excel
...
tauri-app/
├── .gitignore
├── app-icon.png           # исходная иконка проекта
├── LICENSE
...
│   └── libs/
│       └── exceljs.min.js  # локальная minified-библиотека (в .gitignore)
...
│   │   ├── icon.png
│   │   ├── icon.icns
│   │   └── (другие desktop-иконки и размеры)
...
- [x] Добавить проверку SHA256 exe 
...
- [ ] Подписать exe сертификатом - ага, размечтался...
...
- Переход на экспорт в Excel (`.xlsx`) через `ExcelJS`.
- Улучшена структура и оформление экспортируемого файла: стили, формулы, защита листа, блоки Z7.
- Усилена защита экспорта: санитизация текстовых значений для предотвращения Excel-формульной инъекции.
- Доработаны UI и стабильность работы приложения (исправления валидации, сохранения состояния и UX-мелочи).


### v0.6.8

- Релиз версии 0.6.8 с улучшениями стабильности и UX.
- Исправлены критические баги в обработке данных и экспорте.
- Улучшена производительность расчётов при работе с большим количеством операций.
- Обновлена документация и оптимизирована база кода.

### v0.6.7

- Релиз-кандидат перед финальным релизом 0.6.9: небольшие исправления и улучшения.
